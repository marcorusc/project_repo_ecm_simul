...
def write_config_file(name):
    ...
    # print("project_repo_ecm_simul.py: ------- sub.numx, sub.numy = ", sub.numx, sub.numy)


# callback from write_config_button
# def write_config_file_cb(b):
#     path_to_share = os.path.join('~', '.local','share','project_repo_ecm_simul')
...
def get_config_files():
    ...
    if nanoHUB_flag:
        full_path = os.path.expanduser("~/data/results/.submit_cache/project_repo_ecm_simul")  # does Windows like this?
    else:
        # local cache
        try:
            cachedir = os.environ['CACHEDIR']
            full_path = os.path.join(cachedir, "project_repo_ecm_simul")
        except:
...
def run_done_func(s, rdir):
    ...
    if nanoHUB_flag:
        ...
        os.system("submit  mail2self -s 'nanoHUB project_repo_ecm_simul' -t 'Your Run completed.'&")
...
def run_sim_func(s):
    ...
    if nanoHUB_flag:
        if remote_cb.value:
            s.run(run_name, "-v ncn-hub_M@brown -n 8 -w 1440 project_repo_ecm_simul-r7 config.xml")   # "-r7" suffix??
        else:
...
tool_title = widgets.Label(r'\(\textbf{project_repo_ecm_simul}\)')
if nanoHUB_flag or hublib_flag:
